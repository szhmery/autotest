...
static bool		s_ICMPEnable				= true;			// enable udp packets to diff
static u32		s_ICMPLengthMin				= 16;			// set a default min icmp length
static u32		s_ICMPLengthMax				= 8192;			// max length to be consider
...
static s64		s_LatencyHistoMin			= -1e9;			// delat min value
static s64		s_LatencyHistoMax			=  1e9;			// delat max value
...
static char		s_FileName[128]				= "latency.txt";			// file name to store latency data
//---------------------------------------------------------------------------------------------
...
static u128 PayloadHash(u8* Payload, u32 Length)
{
	...
	*/
	...
	{
		Hash = ((Hash << 5ULL) ^ (Hash >> 123ULL)) ^ Payload[i];
...
static void TracePacket(HashNode_t* N)
{
	...
	for (; c >= 0; c--)
	{
		...
		case PKTTYPE_UDP:
		{
			...
			printf("%3i.%3i.%3i.%3i -> %3i.%3i.%3i.%3i | UDP %05i %05i (+%6lli us)",
...
static void NodeOutput(HashNode_t* N)
{
	...
	if (s_EnableFileDiff)
	{
		...
		//printf("%lli ns %016llx %016llx : %i %i  DoSample: %d\n", dT, TS0, TS1, FID0, FID1, DoSample);
	}
	// diff within a file
	else if (N->Cnt >= 2)
	{
	...
	if (DoSample)
	{
		...
		if ((FID0 != -1) && (FID1 != -1) )
		{
			...
			if(dT > 0){
				...
				//printf("%lli us %016llx %016llx : %i %i\n", dT, TS0, TS1, FID0, FID1);
			}
...
static void HashPacket(u32 FID, PCAPPacket_t* Pkt, u32 Type, u128 Hash, u32 Length)
{
    ...
    printf("New packet: NIndex: %d  Hash:%016llx_%016llx Type:%d\n", NIndex, (u64)(Hash>>64), (u64)Hash, Type);
	if (NIndex != 0)
	{
		HashNode_t* NS = &s_HashList[NIndex];
		{
			...
			while (NS)
			{
			    printf("Old packet: NS hash: %016llx_%016llx Type:%d s_HashMatchMAC: %d\n",(u64)(NS->Hash>>64), (u64)NS->Hash, NS->Type, s_HashMatchMAC);
				if ((NS->Hash == Hash) && 
					(NS->Type == Type) &&
					((!s_HashMatchMAC) | (NS->MAC == MAC[0]))
				){
                    printf("NIndex: %d is match\n", NIndex);
				    // can delete previous entries
					s64 dT = TS - NS->TS[0];
					if (abs(dT) > s_TimeDeltaMaxNS)
					{
						printf("kick sampl: dT %lli  time: %s\n", dT,FormatTS(TS));
						// kick sample
						NodeOutput(NS);
					...
					{
						NS->Cnt++;
						printf("kick sampl: NS->Cnt %d dT %lli time: %s\n", NS->Cnt, dT, FormatTS(TS) );
	...
	if (!NodeHit)
	{
		...
		u32 Nindex_tmp = N - s_HashList;

		printf("Save Nindex %d\n",Nindex_tmp);
	}
...
static void ICMPProcess(u32 FID, PCAPPacket_t* Pkt, fEther_t* E, IP4Header_t* IP4, u32 IPOffset)
{
	if (!s_ICMPEnable) return;

	ICMPHeader_t* ICMP 	= (ICMPHeader_t*)( ((u8*)IP4) + IPOffset);
	u8* Payload			= (u8*)(ICMP + 16);
	u32 Length 			= swap16(IP4->Len) - 20 - 16;
	//printf("s_ICMPEnable:  Length %d\n",Length);

	if (Length > 16*1024)
	{
		fprintf(stderr, "ICMP length bogus %i\n", Length);
		return;
	}
	if ((Length >= s_ICMPLengthMin) && (Length <= s_ICMPLengthMax))
	{
		// generate payload hash
		u128 Hash = PayloadHash(Payload, Length);
		HashPacket(FID, Pkt, PKTTYPE_UDP, Hash, Length);
...
{
	fEther_t* E = PCAPETHHeader(Pkt);
	IP4Header_t* IP4n = PCAPIP4Header( Pkt );
	UDPHeader_t* UDPn = PCAPUDPHeader( Pkt );

	printf("ProcessHashPayload %3i.%3i.%3i.%3i -> %3i.%3i.%3i.%3i | UDP %05i %05i (TS %lli ns Time: %s)\n",
	...
	case ETHER_PROTO_IPV4:
	{
		...
		case IPV4_PROTO_ICMP: ICMPProcess(FID, Pkt, E, IP4, IPOffset);break;
		}
	}
	break;
...
static void PrintLatencyHisto(u32 PCAPCnt, PCAPFile_t* PCAPFile[])
{
	...
	for (int i=HMin; i <= HMax; i++)
	{
		...
		//26800 us :            1 0.1667 (1.0000) : ********
		printf("%8lli us : %12lli %.4f (%.4f) (%.4f) : ", dT, s_LatencyHisto[i], Pct, Sum / (double)Total, s_LatencyHisto[i]/(double)Total);
        ValueSum += s_LatencyHisto[i] * dT;
        if (s_LatencyHisto[i] > MaxCount) {
            MaxCount = s_LatencyHisto[i];
            MaxDt = dT;
        }
		int StarCnt = 50*Pct;
	...
	printf("\n");
    u64 w_average = ValueSum/Total;
    double w_average_ms = w_average/(double)1000;
    double MaxDt_ms = MaxDt / (double)1000;
    printf("weighted average: %.3f\n",w_average_ms);
    printf("max count latency: %.3f\n",MaxDt_ms);
//    FILE *p = fopen(s_FileName, "a+");
//    if (p == NULL)
//    {
//        printf("error is %s\n", strerror(errno));
...
static void print_usage(void)
{
	...
	printf(" --store-file                    | The file name to save latency\n");
	printf("\n");
}
...
int main(int argc, char* argv[])
{
	...
	for (int i=1; i < argc; i++)
	{
		...
		else
		{
			...
			else if (strcmp(argv[i], "--store-file") == 0)
			{
				strcpy(s_FileName, argv[i+1]);
				fprintf(stdout,"Filename is %s.",s_FileName);
				fprintf(stderr, "save latency data to file name %s\n", s_FileName);
				i+= 1;
			}
			else
			{
	...
	while (true)
	{
		...
		for (int FID=0; FID < FileNameListPos; FID++)
		{
			...
			while (true)
			{
				...
				{
					PCAP->Finished = true;
					printf("Finished to read FID %d file.", FID);
					break;
				}
			...
			printf("FID %d file has %lli packets. TotalPkt %lli ", FID, PCAP->PktCnt, TotalPkt);
		}
